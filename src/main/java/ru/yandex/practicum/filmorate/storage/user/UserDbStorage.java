...
public class UserDbStorage implements UserStorage {
    ...
    private final String INSERT_USER = "INSERT INTO users (email, login, name, birthday) VALUES (?, ?, ?, ?)";
    ...
    private final String FIND_ALL_FRIENDS = "SELECT u.*\n" +
            "FROM users u\n" +
            "JOIN friendships f ON u.id = f.friend_id\n" +
            "WHERE f.user_id = ?;";
    private final String ADD_FRIEND = "INSERT INTO friendships (user_id, friend_id, is_confirmed) VALUES (?, ?, ?)";
    private final String UPDATE_FRIENDSHIPS_IS_CONFIRMED = "UPDATE friendships SET is_confirmed = ? WHERE user_id = ? AND friend_id = ?";
    private final String DELETE_FRIEND = "DELETE FROM friendships WHERE user_id = ? AND friend_id = ?";
    private final String COMMON_FRIEND = "SELECT u.*\n" +
            "FROM users AS u\n" +
            "WHERE u.id IN (\n" +
